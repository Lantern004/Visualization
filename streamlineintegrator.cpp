...
StreamlineIntegrator::StreamlineIntegrator()
    ...
    , propStepNum("stepNum", "Step Numbers", 30, 1, 100)
    , propStepSize("stepSize", "Step Size", 0.25, 0.01, 0.5)
    , propForward("forward", "Forward", true)
    , propNormalized("normalized", "Normalized", true)
...
void StreamlineIntegrator::process() {
    ...
    if (propSeedMode.get() == 0) {
        ...
        int stepNum = propStepNum.get();
        double stepSize = propStepSize.get();
        int direction = propForward.get();
        int normal = propNormalized.get();
        // int direction = 1;
        vec4 red = vec4(1, 0, 0, 1);
        auto indexBufferLine = mesh->addIndexBuffer(DrawType::Lines, ConnectivityType::Strip);

        dvec2 nextPoint = startPoint;
        Integrator::drawNextPointInPolyline(nextPoint, red, indexBufferLine.get(), vertices);
        for (int i = 0; i < stepNum; i++) {
            nextPoint = Integrator::RK4(vectorField, nextPoint, stepSize, direction, 0);
            
            if (BBoxMin_[0] < nextPoint[0] &&  BBoxMin_[1] < nextPoint[1] && nextPoint[0] < BBoxMax_[0] && nextPoint[1] < BBoxMax_[1])
            {
                Integrator::drawNextPointInPolyline(nextPoint, red, indexBufferLine.get(), vertices);
            } else
            {
